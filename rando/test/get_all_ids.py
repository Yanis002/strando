...
class SpoilerLogEntry:
    ...
    def export(self):
        ...
        if len(self.actors) > 0:
            ...
            node.locations.extend(
                [
                    LocationDef(
                        entry.id_hash,
                        f"NPCA - {entry.id};{entry.x};{entry.y} - Params: {';'.join(f'0x{e:02X}' for e in entry.params)}",
                    )
        ...
        if len(self.mapobj) > 0:
            ...
            node.locations.extend(
                [
                    LocationDef(
                        entry.id_hash,
                        f"MPOB - {entry.id};{entry.tile_x};{entry.tile_y} - Params: {';'.join(f'0x{e:02X}' for e in entry.params)}",
                    )
                    for entry in self.mapobj
                ]
            )
